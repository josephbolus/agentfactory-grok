...
func normalizeTask(input protocol.SaveTaskRequest, now time.Time) (normalizedTask, error) {
	...
	if value.outcomeContract == protocol.OutcomeAgentUpdate {
	...
	if value.outcomeContract != "" && value.outcomeContract != protocol.OutcomeProcessExit {
		return value, invalid("invalid_outcome_contract", "outcome_contract must be process_exit; agent_update was removed")
	}
...
func validateTaskWorkflow(ctx context.Context, tx *sql.Tx, repositoryIDs []string, workflowID string) error {
	if workflowID == "" || workflowID == "procedure" {
		return nil
	}
...
func (s *Store) SetTaskOutcomeContract(
	...
	if input.OutcomeContract == protocol.OutcomeAgentUpdate {
		input.OutcomeContract = protocol.OutcomeProcessExit
	}
	if input.OutcomeContract != protocol.OutcomeProcessExit {
		return protocol.Task{}, invalid("invalid_outcome_contract", "outcome_contract must be process_exit; agent_update was removed")
	}
	if input.ExpectedGeneration < 1 {
		return protocol.Task{}, invalid("task_generation_required", "expected_generation is required")
	}
...
func (s *Store) admitTask(
	...
	if snapshot.Workflow.ID == "" {
		snapshot.Workflow = protocol.ProcedureWorkflowSnapshot(snapshot.Name, snapshot.Prompt)
	...
	execution.Backend = protocol.BackendPersistent
	...
	for position, repository := range snapshot.Repositories {
		...
		if snapshot.WorkflowID != "" && snapshot.WorkflowID != "procedure" {
		...
		if workflow.ID == "" {
			workflow = protocol.ProcedureWorkflowSnapshot(snapshot.Name, resolvedPrompt)
		}
		targetPrompt := resolveTaskWorkflowPrompt(resolvedPrompt, workflow, snapshot.WorkflowID != "" && snapshot.WorkflowID != "procedure")
...
func resolveSessionStages(
	...
	if !protocol.AgentPromptFits(task.Name, target.RepositoryIdentity, prompt) {
		return nil, conflict("agent_prompt_too_large", "the frozen workflow cannot fit the Worker request")
	}
	return []protocol.StageRun{{
		Position: 0, Name: "Do the work", Prompt: prompt, State: protocol.StagePending,
	}}, nil
}
...
func (s *Store) Run(ctx context.Context, id string) (protocol.RunDetail, error) {
	...
	for index := range detail.Sessions {
		...
		session.Stages = nil
		if session.Workflow.ID == "" {
			session.Workflow = protocol.ProcedureWorkflowSnapshot(detail.Run.Task.Name, session.ResolvedPrompt)
		}
