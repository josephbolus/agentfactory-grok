...
func Build(ctx context.Context, options Options) error {
	...
	for _, target := range targets {
		...
		ldflags := strings.Join([]string{
			"-s", "-w", "-buildid=",
			"-X", "github.com/josephbolus/agentfactory-grok/internal/buildinfo.Version=" + options.Version,
			"-X", "github.com/josephbolus/agentfactory-grok/internal/buildinfo.Commit=" + options.Commit,
		}, " ")
...
func renderSPDX(version, commit string, sourceTime time.Time, archive, digest string, modules []module, npmPackages []npmPackage) ([]byte, error) {
	...
	document := spdxDocument{
		...
		DocumentNamespace: "https://github.com/josephbolus/agentfactory-grok/releases/download/" + version + "/" + archive + ".spdx.json",
		...
		Packages: []spdxPackage{{
			...
			DownloadLocation: "https://github.com/josephbolus/agentfactory-grok/releases/download/" + version + "/" + archive,
			FilesAnalyzed:    false, LicenseConcluded: "MIT", LicenseDeclared: "MIT",
			Checksums:    []spdxChecksum{{Algorithm: "SHA256", ChecksumValue: digest}},
			ExternalRefs: []spdxRef{{ReferenceCategory: "OTHER", ReferenceType: "vcs", ReferenceLocator: "git+https://github.com/josephbolus/agentfactory-grok@" + commit}},
		}},
