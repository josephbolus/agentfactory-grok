# Upgrade existing Agent Factory data

Agent Factory keeps existing Task, Attempt, Runbook, Automation, and Occurrence history
...
Back up the Agent Factory data directory. Finish any active legacy poller import
...
1. Select **Upgrade Agent Factory** on Overview. If legacy executions are active,
   choose **Freeze and drain** or **Freeze and cancel active work**.
2. Agent Factory freezes legacy Task, Runbook, and polling mutations before waiting.
   New Definitions and Runs remain available.
3. When active legacy work is terminal, continue the upgrade. Agent Factory converts
   compatible schedules in one SQLite transaction.
4. Review old Tasks under **Work**, now labelled **Legacy task history**.

The freeze is durable. If Agent Factory stops after freezing but before conversion,
