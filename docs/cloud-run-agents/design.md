...
Agent Factory currently runs every coding agent through a persistent Worker on a
...
This design adds Cloud Run Jobs as a second execution backend. Agent Factory will
...
workload credentials to Google client libraries. Agent Factory calls Cloud Run and the
...
artifact before Agent Factory accepts completion. Cloud Run also isolates a process
...
The Cloud Run experiment proved a narrower path against the Agent Factory repository:
...
That experiment proves container execution, not the Agent Factory lifecycle. It does
...
flowchart LR
    ...
    R --> CP["Agent Factory control plane"]
...
Agent Factory owns Tasks, Runs, Sessions, and Attempt identity, the frozen prompt and
...
| --- | --- | --- |
| Execution backend | Persistent Worker, Cloud Run Job | Agent Factory routing |
...
Agent Factory. This preserves the current architecture while preventing an agent
...
`Cloud Run Europe` profile as a manual run override. Agent Factory creates ordinary
Run and Session records. A cloud repository input resolver turns the configured
source ref into a full Git commit SHA before the Session queues. Agent Factory freezes
...
starts in `dispatching`. Agent Factory writes the immutable input and gateway
...
object. Agent Factory records local monotonic time immediately before that gateway
...
later gateway responses are delayed or unavailable. Agent Factory calls one immutable,
...
has no invoker grant on another Agent Factory service. The gateway requires the exact
...
If a lost API response causes Agent Factory to launch a duplicate, only one execution
...
evidence, restart the agent, or perform repository tool actions. Agent Factory's
...
This preserves
fail-closed lease behavior without making the local Agent Factory API reachable from
the internet.
Cloud execution therefore depends on one continuously running Agent Factory control
...
Agent Factory polls and ingests those batches idempotently into the existing Attempt
...
output. It is written last. Agent Factory rejects an object or manifest whose storage
...
verification checkout at the frozen commit, Agent Factory applies a patch with
...
counts, and excess expanded bytes. Agent Factory then compares the reconstructed Git
...
Cancellation first records Agent Factory's intent and revokes authority. The
...
account, then asks Agent Factory to validate or provision the managed resources. The
...
and model at admission. A retry cannot change backend, and Agent Factory does not
...
Agent Factory rejects a set `GOOGLE_APPLICATION_CREDENTIALS` variable and a local ADC
...
dispatcher subject. Agent Factory rejects `service_account` key files and plain
...
mode makes `gcloud` an Agent Factory runtime dependency. User-managed service-account
...
Agent Factory uses access tokens with Google client libraries and the Cloud Run v2
...
| --- | --- | --- | --- |
| Provisioning operator or temporary provisioner | None managed by Agent Factory after setup validation | Setup session only | Credentials in Agent Factory, gateway, Job, config, or SQLite |
...
| Gateway service account | `roles/storage.objectAdmin` | Dedicated artifact bucket | Cloud Run Job administration, model secrets, Agent Factory operator API |
| Profile Job service account | `roles/run.servicesInvoker`; `roles/secretmanager.secretAccessor` | One gateway service; one dedicated model secret | Artifact bucket, Cloud Run administration, Agent Factory operator API, sibling model secrets |

These are the complete Agent Factory-managed bindings. A runtime principal with any
...
agents are outside this table and never run Agent Factory code. The provisioning
operator's pre-existing human administration is part of the deployment trust
boundary, not an Agent Factory runtime identity.
...
permission plan before applying it. Agent Factory never stores that identity. If a
...
role](https://docs.cloud.google.com/run/docs/reference/iam/roles), but Agent Factory
...
bounded output publication. The dispatcher surface authenticates Agent Factory's
...
status. It does not choose work, retry an Attempt, or decide Agent Factory's terminal
outcome.
...
It is transport and recovery storage, not the source of Agent Factory lifecycle
truth. Agent Factory remains able to rebuild its view from SQLite and reconcile
...
public callback into Agent Factory because the operator API is loopback-only and the
current product has no tenant authentication boundary. A later hosted Agent Factory
deployment may replace polling with an authenticated private callback or queue
without changing Attempt identity.

#### Agent Factory owns retries

Cloud Run task retries are zero. Every operator retry creates a new Agent Factory
...
have made external side effects and Agent Factory must preserve one visible retry
history.
...
diagnosis. Ordered event objects and the verified final manifest drive Agent Factory
state. We reject parsing Cloud Logging as the event protocol because delivery,
ordering, retention, and redaction are controlled outside Agent Factory.

#### Trusted repositories first

The first release supports repositories and prompts trusted by the Agent Factory
...
explicit retry uses the same version; if it is unavailable, Agent Factory blocks the
retry instead of silently running different code.

#### Direct APIs with attached workload identity

Agent Factory uses Google client libraries and direct v2 APIs. Hosted mode obtains
...
metadata server. We also defer hosting Agent Factory itself on Cloud Run because that
...
- `INV-1`: Agent Factory is the sole authority for Run, Attempt, retry,
...
- `INV-5`: Cloud Run task retries remain zero; Agent Factory creates every retry.
- `INV-6`: Event ingestion is ordered and idempotent by Attempt, run ID,
  sequence, and event ID.
- `INV-7`: Agent Factory accepts successful completion only after verifying the final
...
- `INV-14`: The gateway refuses to start an agent at or after the frozen
  Session deadline. Agent Factory times out and cancels an accepted execution that has
...
- `INV-15`: Hosted Agent Factory authenticates Google runtime calls only through the
...
- The immutable input includes `timeout_seconds`. The gateway start-fence
  ...
  deadline Agent Factory records timeout intent in SQLite, while the wrapper applies
...
- The hosted Agent Factory VM has one dedicated attached dispatcher service account,
...
- Hosted Agent Factory rejects environment or well-known-file ADC and calls the Cloud
...
`attempt_id` is the Agent Factory identity. `run_id` is random, non-secret, and
immutable for one dispatch. It is safe for object names, events, manifests,
logs, and diagnostic URLs. Agent Factory envelope-encrypts the separate raw run
...
inspect model credentials. Agent Factory never stores raw plaintext: SQLite stores
...
Google and never replace Agent Factory identity. Missing names keep the record in
...
Agent Factory is the only state-machine writer. GCS objects and Cloud Run status are
evidence that permit one Agent Factory transition; they never transition a Run by
themselves.

| Agent Factory dispatch state | Required evidence | Allowed next states |
| --- | --- | --- |
...
| `terminal` | One stored Agent Factory outcome | none |
...
call, Agent Factory obtains a Cloud Run provider timestamp from the preceding API
...
this cutoff from the Agent Factory clock. If any Run call lacks a trustworthy provider
timestamp, Agent Factory durably sets `provider_search_unbounded`; that flag never
...
The Agent Factory Attempt and run ID provide deterministic dispatch identity.
Agent Factory stores every operation or execution name observed for that run ID
...
evidence and first obtains a fresh gateway authority-state response. Agent Factory
...
Session deadline remains unexpired, Agent Factory leaves completion in `reconciling`
...
the first durable Agent Factory decision. If timeout intent wins, a later explicit
...
expected generation, but no caller-supplied time. Agent Factory allows only one
...
historical evidence, not current authority state, and Agent Factory never treats its
expected generation as a current generation. Agent Factory ignores such a replay after
timeout or cancellation intent owns the outcome. Agent Factory cannot issue the next
...
operation. The wrapper and Agent Factory record suspend-aware monotonic instants both
...
pre-request instant. Agent Factory separately persists an authority-expiry upper
...
window; neither bound can let Agent Factory terminalize while gateway authority may
still be valid. Neither the Agent Factory clock nor the wrapper wall clock can extend
...
control-plane authority-revocation compare-and-set requested by Agent Factory.

Agent Factory asks the gateway to refresh authority only while the same Attempt lease
and dispatch record remain active. Every gateway response includes server time.
Agent Factory records local monotonic time immediately before every request and
...
whether an accepted execution may continue; neither Agent Factory wall time nor a
delayed response can extend the Session. Once that deadline arrives, Agent Factory
...
generation conflict, Agent Factory reads the new generation and retries revocation; it
...
is unreachable, Agent Factory does not treat the last acknowledged lease alone as
...
expiry of a conservative authority-uncertainty deadline permits Agent Factory to
...
When one refresh may be unacknowledged, Agent Factory adds one full 30-second
...
restate expiry. Agent Factory durably records which proof terminalized the outcome,
then requests Cloud Run cancellation as platform cleanup. Therefore no container
can retain valid authority after Agent Factory records the terminal timeout or
cancellation. After an Agent Factory restart, no refresh or Run retry is allowed until
a fresh gateway response establishes a new conservative monotonic deadline. If
the gateway is unreachable, Agent Factory first proves exclusive dispatch ownership
...
execution must be terminal; otherwise Agent Factory uses revocation or authority-
...
explicit: a continuously available Agent Factory controller is required for a
continuously running Cloud Job.

After Agent Factory commits explicit cancellation, it asks the gateway to revoke
...
execution terminal, so Agent Factory tracks that platform reconciliation separately
...
reason. Agent Factory does not try another local credential, invoke `gcloud`, read a
key file, or fall back to a broader identity.

If access-token or ID-token acquisition fails while an Attempt is active,
Agent Factory stops new Run and authority-refresh calls and moves the dispatch to
reconciling. The wrapper stops when its last verified authority expires.
Agent Factory retries profile validation no more than once every 30 seconds with
...
or dispatch after the VM restarts. As a defensive runtime check, Agent Factory still
...
the next refresh. Agent Factory keeps the profile disabled until the stored expected
...
If the Run API accepts a request but its response is lost, Agent Factory keeps the
...
Agent Factory lease and cloud authority only while gateway time remains before the
persisted `session_deadline_at` and Agent Factory's conservative monotonic deadline
has not expired. The gateway caps every authority lease at that deadline. When
the monotonic deadline arrives, Agent Factory records timeout intent and conditionally
...
nonzero without attempting a final event. Agent Factory's durable record supplies the
...
batches. Agent Factory accepts a repeated batch only when its identity and bytes
...
If Agent Factory loses Google API access temporarily, it keeps the dispatch in
...
agent. Agent Factory cancels the cloud execution when access returns and fails the
Attempt rather than reviving it.

If Agent Factory restarts, it loads every nonterminal dispatch before admitting more
...
If cancellation or timeout races with completion, the first durable Agent Factory
...
Cloud Run cancellation, and waits for up to 30 seconds. On restart, Agent Factory
...
The initial trust boundary is one trusted Agent Factory operator, trusted repository
...
Agent Factory control-plane credential.
Repository credentials are short-lived and scoped to one repository when
private checkout or publishing is added.

Agent Factory's runtime image and service definition do not require the Google Cloud
CLI. Provisioning may use `gcloud`, but it cannot copy its user configuration
or tokens into the Agent Factory service account. The deployment creates no
...
Completed artifact prefixes use a configured retention period. Agent Factory keeps
...
- `AC-4`: After Agent Factory commits cancellation, it revokes authority within five
...
- `AC-5`: Agent Factory restart reconciles every nonterminal execution before it
  admits new cloud Runs.
- `AC-6`: Native Cloud Run retries are zero and only an explicit Agent Factory retry
  creates another Attempt.
- `AC-7`: Events remain ordered, bounded, and duplicate-safe across delayed or
  repeated object ingestion.
- `AC-8`: Success requires Agent Factory to reconstruct a verified final manifest and
...
- `AC-14`: Cloud Run startup, checkout, and agent execution share one frozen
  Session timeout established before the Run call. At its deadline Agent Factory times
...
- `AC-15`: On a dedicated Compute Engine VM with the configured dispatcher
  service account attached, Agent Factory can read and run the versioned Job, inspect,
...
missing or invalid encryption keys, Agent Factory clocks ahead of and behind the
...
deadline. A delayed gateway response that shortens Agent Factory's monotonic deadline
...
or authority deadlines. Lost-launch tests skew the Agent Factory clock in both
...
Agent Factory waits beyond the real lease plus its possible extension. A separate
restart test discards the old monotonic anchor, makes the gateway unreachable,
and proves terminalization remains blocked until Agent Factory acquires exclusive
...
deadline, replaying its request ID cannot extend authority again, and Agent Factory
...
Agent Factory must ignore it for state transitions. The same replay must run after
...
`AC-13`. They will verify that raw capabilities never appear in Agent Factory
...
manifest, artifact, or other gateway write, while still permitting Agent Factory's
...
Agent Factory stops refresh within ten seconds of the first response. A separate
...
- Should a hosted Agent Factory deployment replace storage polling with a private
