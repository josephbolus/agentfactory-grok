# GitHub issue ingest for Agent Factory
...
> belong to the [Software Agent Factory target architecture](../software-factory/design.md).
...
Agent Factory previously had a simple `factory-poller` process that submitted a
...
workers. Current Agent Factory instead runs typed GitHub Automation evaluation and
durable deduplication inside the one control-plane process.

This design extends the current GitHub issue path:

```text
matching GitHub issue -> Agent Factory task -> selected worker -> agent-managed issue and PR
```
...
flowchart LR
    ...
    I -->|"POST /api/v1/tasks"| CP["Agent Factory control plane"]
...
client. This reuses local authentication and keeps credentials out of Agent Factory.
...
highest Agent Factory-owned instruction.
