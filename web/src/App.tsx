import { Anvil, Bot, Columns3, Gauge, GitBranch, Menu, Plus, Repeat2, X } from "lucide-react";
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <svg className="canvas-flow" aria-hidden="true">
      ...
      <path className="canvas-flow-pulse" d="M 32 128 H 2400 V 384 H 320 V 1600" />
      <path className="canvas-flow-wire" d="M 160 32 V 1800 H 1280 V 256 H 2400" />
      <path className="canvas-flow-pulse" d="M 160 32 V 1800 H 1280 V 256 H 2400" />
      <path className="canvas-flow-wire" d="M 0 512 H 960 V 96 H 2400" />
      <path className="canvas-flow-pulse" d="M 0 512 H 960 V 96 H 2400" />
    </svg>
    <aside className={`sidebar ${mobileNavOpen ? "sidebar-open" : ""}`}>
      <div className="brand"><div className="brand-mark" aria-hidden="true"><Anvil size={18} strokeWidth={2} /></div><div><span className="brand-name">Agent Factory</span><span className="brand-subtitle">control plane</span></div></div>
      <nav aria-label="Primary navigation">
        <div className="nav-section" role="group" aria-labelledby="work-nav-label">
          ...
          <Nav active={route.page === "work" || route.page === "run-detail"} icon={<Columns3 size={16} strokeWidth={2} />} label="Work" onClick={() => navigate({ page: "work", mode: activeRunMode })} />
          <Nav active={route.page === "tasks"} icon={<Repeat2 size={16} strokeWidth={2} />} label="Tasks" onClick={() => navigate({ page: "tasks" })} />
          <Nav active={route.page === "overview"} icon={<Gauge size={16} strokeWidth={2} />} label="Overview" onClick={() => navigate({ page: "overview" })} />
          </div>
        </div>
        <div className="nav-section" role="group" aria-labelledby="infrastructure-nav-label">
          ...
          <div className="nav-items">
            <Nav active={route.page === "workers" || route.page === "worker"} icon={<Bot size={16} strokeWidth={2} />} label="Workers" onClick={() => navigate({ page: "workers" })} />
            <Nav active={route.page === "repositories" || route.page === "repository"} icon={<GitBranch size={16} strokeWidth={2} />} label="Repositories" onClick={() => navigate({ page: "repositories" })} />
          </div>
        </div>
      </nav>
    ...
    <div className="main-shell">
      <header className="topbar"><button className="icon-button mobile-menu" aria-label="Toggle navigation" aria-expanded={mobileNavOpen} onClick={() => setMobileNavOpen((open) => !open)}>{mobileNavOpen ? <X size={16} strokeWidth={2} /> : <Menu size={16} strokeWidth={2} />}</button><div className="topbar-title">{pageTitle(route)}</div><button className="button button-primary" onClick={() => navigate({ page: "tasks", create: true })}><Plus size={16} strokeWidth={2} /> New Task</button></header>
