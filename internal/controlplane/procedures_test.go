...
func TestProcedureRunFreezesAllEnabledRepositoriesAndReplaysBeforeChanges(t *testing.T) {
	...
	for index, work := range admission.Run.Sessions {
		...
		if len(storedWork.Stages) != 0 || storedWork.Workflow.ID == "" || storedWork.Workflow.Instructions == "" {
	...
	if err != nil || claim == nil || len(claim.Session.Stages) != 0 || claim.Session.Workflow.ID == "" {
		t.Fatalf("fleet workflow claim = %#v, err %v", claim, err)
	}
