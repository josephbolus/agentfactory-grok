...
Agent Factory keeps its browser and operator API on loopback. Remote VMs connect to a
...
The saved credential is bound to the exact Agent Factory server origin. Changing the
...
authenticated Worker identity. Agent Factory does not provision VMs, distribute
