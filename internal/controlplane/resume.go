...
func validateContinuationWithinTx(
	...
	if err != nil || !protocol.AgentPromptFits(
		state.title, state.repository, prompt,
	) {
		return &ServiceError{
...
func assembleContinuationPrompt(state continuationState, history []continuationHistory) (string, error) {
	...
	baseBytes := len([]byte(protocol.FormatAgentPrompt(
	...
	if !protocol.AgentPromptFits(state.title, state.repository, prompt) {
		return "", &ServiceError{
			Code:    "continuation_prompt_too_large",
			Message: "the mandatory recovery context cannot fit the 72 KiB agent prompt",
			Status:  413,
...
func (s *Store) AnswerWork(
	...
	if backend != protocol.BackendPersistent {
		return protocol.WorkAnswer{}, conflict("persistent_backend_required", "resumable Work requires the persistent execution backend")
	}
