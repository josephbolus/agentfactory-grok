...
if grep -E '(^|[[:space:]])poll(-once|-test)?([[:space:]]|$)' "$root/Makefile" >/dev/null; then
  echo "retired poller command remains in Makefile" >&2
  exit 1
fi
if [ -e "$root/Justfile" ]; then
  echo "Justfile must not exist; Makefile is the only driver" >&2
  exit 1
fi
...
PATH="$temporary/ui-bin:/usr/bin:/bin" \
  make -C "$root" ui-build INSTALL=0
