# Agent Factory command-line interface
...
Agent Factory will ship one Go binary named `factory`. It will preserve the current
...
Without `--workflow`, Agent Factory uses a blank workflow and sends the context as the
task description.
...
Agent Factory keeps one home at `~/.factory`. Proposed client config at
`~/.factory/client.toml`:
