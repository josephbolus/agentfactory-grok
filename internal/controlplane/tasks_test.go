...
func TestRunPagePreservesRepositorySummaryWithoutPrompt(t *testing.T) {
	...
	if len(summary.Repositories) != 1 || summary.Repositories[0].RemoteIdentity != "github.com/josephbolus/agentfactory-grok" {
...
func TestCancelSessionCancelsOnlyTheSelectedSession(t *testing.T) {
	...
	worker := registerTestWorker(t, store, workerA, 10,
		protocol.RepositoryRegistration{Key: "factory", RemoteIdentity: "github.com/josephbolus/agentfactory-grok"},
...
func TestClaimScansPastFiftyHealthyQueuedAssignmentsToReroute(t *testing.T) {
	store := newTestStore(t)
	repository := protocol.RepositoryRegistration{Key: "factory", RemoteIdentity: "github.com/josephbolus/agentfactory-grok"}
