...
test("does not author Pipelines as SDLC", async ({ page }) => {
  await page.goto("/");
  const navigation = page.getByRole("navigation", { name: "Primary navigation" });
  await expect(navigation.getByRole("button", { name: "Work", exact: true })).toBeVisible();
  await expect(navigation.getByRole("button", { name: "Tasks", exact: true })).toBeVisible();
  await expect(navigation.getByRole("button", { name: "Workers", exact: true })).toBeVisible();
  await expect(navigation.getByRole("button", { name: "Repositories", exact: true })).toBeVisible();
  await expect(navigation.getByRole("button", { name: "Pipelines" })).toHaveCount(0);
  await page.goto("/pipelines");
  await expect(page.getByRole("heading", { name: "Pipelines", exact: true })).toHaveCount(0);
...
test("creates a Task and completes its Run", async ({ page }) => {
  ...
  await expect(dialog.getByLabel("Pipeline")).toHaveCount(0);
  ...
  await expect(page.locator(".stage-run")).toHaveCount(0);
