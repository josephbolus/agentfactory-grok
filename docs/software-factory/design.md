...
Agent Factory currently runs saved prompts across repositories, but a developer who
...
references or select a repository fleet, then rely on Agent Factory to queue the
work, assign coding agents, apply consistent instructions, and collect progress
and outcomes in one place.

The design keeps Agent Factory out of the coding-agent business. A capable existing
agent owns refinement, implementation, tests, subagents, review, pull-request
creation, and CI repair. Agent Factory gives the agent a small scoped capability to
report progress, readiness, questions, failure, or no change. Agent Factory owns the
durable outer loop: admission, frozen input, capacity, leases, process safety,
history, and visibility.

The main downside is that V1 trusts an agent's semantic status report. Agent Factory
...
```mermaid
flowchart LR
    O["Developer"] --> CLI["Agent Factory CLI or browser"]
    GH["GitHub issue URL"] --> CLI
    LR["Opaque reference such as LINEAR-123"] --> CLI
    CLI --> CP["Agent Factory control plane"]
    ...
    A --> FT["Attempt-scoped Agent Factory update tool"]
...
immutable Agent Factory-supplied input, update history, and user-visible state. A Worker owns its
...
remain sources of work and delivery state, not Agent Factory databases.
...
opaque reference such as `LINEAR-123` requires `--repo` in V1. Agent Factory does not
...
Agent Factory safety preamble, the frozen standard Build Procedure, repository and
...
its existing exit-based completion. Agent Factory never parses the agent's final
...
Agent Factory freezes the current `bug-fix` Procedure and the enabled managed
...
When an agent reports `needs-input`, Agent Factory stores the exact question and
...
is available. Agent Factory does not keep an idle agent process alive while waiting
for a person.
...
scoped capability. Agent Factory does not implement a competing model loop.
...
The agent reports progress and an Attempt-ending outcome through a typed Agent Factory
...
cost is that a capable but mistaken agent may report `ready` too early. Agent Factory
...
review, PR creation, and CI repair in one Attempt. Agent Factory shows these as
...
Agent Factory never force-pushes or deletes the remote publish branch. A normal
...
request, handle CI, and report through Agent Factory. We reject a skill registry and
...
a transcript parser or an Agent Factory-owned reasoning loop.
...
report includes a PR URL and message. Agent Factory must validate the URL, expected
...
- `INV-10`: Agent Factory never derives semantic success by parsing arbitrary agent
  output.
...
- `INV-14`: Historical Work identifies the exact Procedure, Agent Factory-supplied
  ...
  the agent.
- `INV-15`: Agent Factory does not claim exactly-once external side effects performed
  by an agent.
...
A manual `ready` update is not required to have an Agent Factory worktree or publish
...
the repository base.
Agent Factory never force-pushes or deletes the ref. If the ref moves while an Attempt
...
it never marks stale evidence ready. Agent Factory validates delivery identity, not
...
worktree and a fixed checkpoint reason; Agent Factory does not present a question
...
become final. Agent Factory never presents Work as ready while process ownership is
uncertain.
...
Work; Agent Factory clears operator ownership and records cancelled without waiting
...
an answer, Agent Factory also proves the actual mandatory continuation sections fit;
...
Agent Factory heading. The prompt fills remaining bytes
...
a prompt-integrity boundary, not hostile-code isolation. Agent Factory does not place
...
- An agent may report `ready` incorrectly. Agent Factory stores the exact report,
...
- Should Agent Factory add a bounded final reminder turn when an agent exits without
  an update? Default: fail visibly first and measure frequency because runtime
  support for another turn differs.
- Should a ready update require Agent Factory to query GitHub checks? Default: no.
  The agent owns semantic completion. Agent Factory still validates the URL,
...
- An Agent Factory coding agent, model loop, context manager, or subagent framework.
