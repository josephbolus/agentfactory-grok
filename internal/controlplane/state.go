...
func (s *Store) claimDetail(ctx context.Context, attemptID string) (protocol.Claim, error) {
	...
	row = s.db.QueryRowContext(ctx, `
		SELECT session.id, session.run_id, json_extract(run.task_snapshot, '$.name'),
		       session.resolved_prompt, session.repository_workflow_snapshot, session.repository_id, session.timeout_seconds,
	...
	if err = row.Scan(&claim.Session.ID, &claim.Session.RunID, &claim.Session.TaskName,
		&claim.Session.Prompt, &workflowSnapshot, &claim.Session.RepositoryID, &claim.Session.TimeoutSeconds,
	...
	if len(workflowSnapshot) != 0 && string(workflowSnapshot) != "{}" {
		if err := json.Unmarshal(workflowSnapshot, &claim.Session.Workflow); err != nil {
			return claim, unavailable(err)
		}
	}
	...
	claim.Session.Stages = nil
	claim.Session.OutcomeContract = protocol.OutcomeProcessExit
	if claim.Session.Workflow.ID == "" {
		claim.Session.Workflow = protocol.ProcedureWorkflowSnapshot(claim.Session.TaskName, claim.Session.Prompt)
	}
