...
- an internal-use license and confidentiality notice;
...
3. Treat committed files as the source of truth. Avoid duplicating maintained
   instructions across the README, architecture, team workflow, agent
   files, and wiki.
...
Detect real commands from manifests, task runners, workflow docs, and
...
- `LICENSE` with an internal-use notice;
- documented local setup, checks, and pull request expectations for employees;
- `SECURITY.md` for projects handling credentials, personal
...
`ARCHITECTURE.md` for system facts and workflow documentation for setup steps.
