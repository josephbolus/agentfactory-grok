...
An Agent Factory Worker is one stable identity, a set of coding-agent capabilities,
...
Profiles select the adapter, provider, model, and reasoning effort used for
planning, implementation, and independent review. Provider and model values
remain literal: Pi owns provider configuration and credentials. Every profile
adapter must also appear in `runtimes`.

Agent Factory passes effort as Codex `model_reasoning_effort`, Claude Code `--effort`,
and Pi `--thinking`. Accepted values are `off`, `minimal`, `low`, `medium`,
`high`, `xhigh`, and `max`.
...
All roles start at `medium`. A reviewer finding raises the repair executor and
final reviewer to `high`; remaining findings fail the Work. The planner writes
and commits a validated `.factory/plans/` Markdown artifact before implementation.
For GitHub Issue Work, Agent Factory keeps one marked Issue comment with the Worker,
all role profiles, lifecycle state, findings, and final verdict. Legacy
`[review]` configuration remains supported.

When `data_directory` is omitted, Agent Factory derives an absolute path beside the
...
name such as `main`, `develop`, or `release/2026.07`; when omitted, Agent Factory uses
