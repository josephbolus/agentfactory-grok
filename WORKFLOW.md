...
Agent Factory uses the approved internal issue tracker as the source of truth for
planned work. Its project records each issue's lifecycle.
...
| Label | Description |
| --- | --- |
| `needs-agent` | The next action belongs to an agent. For a managed repository with issue intake enabled, Agent Factory polls this label every 30 seconds; it normally accompanies Ready. |
...
Agent Factory creates Work only after the repository is explicitly opted into issue
intake in Infrastructure. An agent re-checks both `needs-agent` and Ready before
it acts. A `needs-agent` label on a pull request can continue only Work already
linked to that pull request; Agent Factory never merges it.
