...
Agent Factory currently asks an operator to understand Definitions, Runbooks,
...
repository set into one Run record. Agent Factory creates one Session per repository
...
`provider_history` source so Agent Factory preserves provenance without restoring
provider admission.
...
current time. Agent Factory therefore skips cron instants missed during downtime or
...
remains untouched. Agent Factory reports the exact blocking records and backup path.
