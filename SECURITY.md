...
[jbolus@owens.com](mailto:jbolus@owens.com) with the affected revision,
...
Agent Factory is under active development. Only the latest commit on `main` receives
...
Agent Factory is a local control plane. It binds to loopback, has no authentication,
...
The allowlist controls Agent Factory assignment and worktree creation. It does not
...
Agent Factory state defaults to `~/.factory`. Protect this directory because it may
contain:
...
Provider CLIs own their credentials. Agent Factory does not request or persist
...
use by Owens OnLine employees only.
