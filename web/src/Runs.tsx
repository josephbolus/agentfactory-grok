...
export function RunDetailView({ id, onBack }: { id: string; onBack: () => void }) {
  ...
  return <div className="page run-detail-clean">
    ...
    <section className="run-summary-strip"><div><span>Procedure</span><strong>{run.task.name}</strong></div><div><span>Workflow</span><strong>{run.task.workflow_id ?? "implement"}</strong></div><div><span>Completed</span><strong>{successfulSessions(run)}</strong></div><div><span>Duration</span><strong>{run.terminal_at ? duration(run.admitted_at, run.terminal_at) : "Active"}</strong></div></section>
