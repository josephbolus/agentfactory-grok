...
func TestBuildAdmissionCreatesIndependentAtomicWorkAndSchedulerClaimsIt(t *testing.T) {
	...
	defaultPipeline, err = store.UpdatePipeline(context.Background(), defaultPipeline.ID, protocol.SavePipelineRequest{
		...
		Stages: []protocol.PipelineStage{
			{Name: "Do the work", Prompt: "{{ task.prompt }}"},
		},
	})
	if err != nil {
		t.Fatal(err)
	...
	repository, _, err := store.CreateManagedRepository(context.Background(), protocol.CreateManagedRepositoryRequest{
		RemoteIdentity: "github.com/josephbolus/agentfactory-grok",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	admission, err := store.AdmitBuild(context.Background(), protocol.BuildRequest{
		...
		References: []string{
			"https://github.com/JOSEPHBOLUS/AGENTFACTORY-GROK/issues/341",
			"LINEAR-123",
			"LINEAR-124",
		},
	})
	...
	if admission.Result != protocol.AdmissionAdmitted || admission.Run.Run.SessionCount != 3 ||
		...
		admission.Run.Run.OutcomeContract != protocol.OutcomeProcessExit {
	...
	for index, work := range admission.Run.Sessions {
		...
		if len(storedWork.Stages) != 0 || storedWork.Workflow.ID == "" || storedWork.Workflow.Digest == "" ||
			storedWork.Workflow.Instructions == "" {
			t.Fatalf("Work %d workflow = %#v stages = %#v", index, storedWork.Workflow, storedWork.Stages)
		}
	...
	if got := admission.Run.Sessions[0].Target.SourceKey; got != "github:josephbolus/agentfactory-grok:issue:341" {
	...
	if claim.Session.RunID != admission.Run.Run.ID || claim.Session.Target.TargetKind != "work_item" ||
		claim.Session.OutcomeContract != protocol.OutcomeProcessExit || len(claim.Session.Stages) != 0 ||
		claim.Session.Workflow.ID == "" || claim.Session.Workflow.Instructions == "" {
