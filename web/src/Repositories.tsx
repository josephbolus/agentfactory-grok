...
import type { ManagedRepository, RepositoryWorkflowCatalog } from "./types";
import {
  EmptyState,
  ErrorState,
  InlineError,
...
export function RepositoryDetail({
  ...
  const workflows = useQuery({
    queryKey: ["repository-workflows", id],
    queryFn: () => api.repositoryWorkflows(id),
    refetchInterval: interval,
  });
  const refreshWorkflows = useMutation({
    mutationFn: () => api.refreshRepositoryWorkflows(id),
    onSuccess: (updated) => queryClient.setQueryData(["repository-workflows", id], updated),
  });
  ...
  const toggleIssueIntake = useMutation({
    mutationFn: (enabled: boolean) => api.setRepositoryIssueIntake(id, enabled),
    onSuccess: (updated) => {
      queryClient.setQueryData(["repository", id], updated);
      queryClient.setQueryData<ManagedRepository[]>(["repositories"], (current) =>
        current?.map((item) => item.id === updated.id ? updated : item),
  ...
  return (
    <div className="page detail-page">
      ...
      <InlineError error={toggle.error ?? toggleIssueIntake.error ?? refreshWorkflows.error} />
      ...
      <div className="repository-detail-layout">
        ...
        <aside className="panel repository-profile-panel">
          ...
          <dl className="metadata">
            ...
            <div><dt>Issue intake</dt><dd>{data.issue_intake_enabled ? "Enabled" : "Disabled"}</dd></div>
          ...
          <button
            ...
            onClick={() => toggleIssueIntake.mutate(!data.issue_intake_enabled)}
            disabled={toggleIssueIntake.isPending}
          >
            {data.issue_intake_enabled ? "Disable issue intake" : "Enable issue intake"}
          </button>
          <p className="quiet-copy">Polls open `needs-agent` issues every 30 seconds.</p>
          <div className="profile-divider" />
          <span className="profile-label">Repository ID</span>
          <span className="worker-id" title={data.id}>{data.id}</span>
        </aside>
      </div>
      <section className="panel repository-workflow-panel">
        <PanelHeading
          title="Repository workflows"
          aside={workflows.data ? `${workflows.data.workflows.length} available` : "Not loaded"}
        />
        {workflows.isPending ? <LoadingState label="Loading repository workflows" /> : workflows.error ? (
          <div className="repository-workflow-empty">
            <span>Workflow catalog unavailable. Refresh after the repository is accessible.</span>
            <button className="button button-secondary" onClick={() => void workflows.refetch()}>Retry</button>
          </div>
        ) : workflows.data ? <WorkflowCatalog catalog={workflows.data} pending={refreshWorkflows.isPending} onRefresh={() => refreshWorkflows.mutate()} /> : null}
      </section>
    </div>
  );
}

function WorkflowCatalog({ catalog, pending, onRefresh }: { catalog: RepositoryWorkflowCatalog; pending: boolean; onRefresh: () => void }) {
  return <>
    <div className="repository-workflow-toolbar">
      <span className={catalog.status === "healthy" ? "healthy-text" : "danger-text"}>
        {catalog.status === "healthy" ? `Synced at ${new Date(catalog.synced_at).toLocaleString()}` : catalog.error || `Catalog ${catalog.status}`}
      </span>
      <button className="button button-secondary" onClick={onRefresh} disabled={pending}>
        <RefreshCw size={14} className={pending ? "spin" : undefined} /> {pending ? "Refreshing…" : "Refresh catalog"}
      </button>
    </div>
    {catalog.workflows.length === 0 ? <div className="quiet-empty">No workflows were found.</div> : <div className="repository-workflow-list">
      {catalog.workflows.map((workflow) => <div className="repository-workflow-row" key={workflow.id}>
        <span><strong>{workflow.title}</strong><small><code>{workflow.id}</code> · {workflow.path}</small></span>
        {workflow.labels_all?.length ? <span className="repository-workflow-labels">{workflow.labels_all.map((label) => <span className="tag" key={label}>{label}</span>)}</span> : <span className="quiet-copy">Manual selection</span>}
      </div>)}
    </div>}
    {!!catalog.diagnostics?.length && <div className="repository-workflow-diagnostics">
      <strong>Routing diagnostics</strong>
      {catalog.diagnostics.map((diagnostic) => <a href={diagnostic.issue_url} target="_blank" rel="noreferrer" key={`${diagnostic.issue_number}-${diagnostic.updated_at}`}>
        Issue #{diagnostic.issue_number}: {diagnostic.message}
      </a>)}
    </div>}
  </>;
}
