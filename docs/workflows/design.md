...
> [Software Agent Factory target architecture](../software-factory/design.md)
> supersedes this model for future product work.
>
> For current repository-contained workflow authoring and GitHub label routing,
> see [Repository workflows](repository-workflows.md).

## 1. Executive summary

Agent Factory can delegate one free-text Task to one repository, but operators must
repeat their instructions, run the standalone poller, and inspect process logs
to understand automated work. Agent Factory now has versioned **Workflows** for
...
MVP because it is the model accepted in issue #173 and preserves Agent Factory's
...
The worker adds its existing fixed Agent Factory safety preamble, title, repository
...
Agent Factory performs no interpolation, Markdown rendering, URL expansion, or
...
Before normal dispatch, Agent Factory verifies that a Task already stored under that
...
For a schedule that is overdue at startup, Agent Factory creates one Occurrence for
...
that can call Agent Factory can read Workflow instructions, create Tasks, test
...
and never enter Agent Factory configuration, API payloads, or SQLite. Missing or
...
full documented checks must pass.
