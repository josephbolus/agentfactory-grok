...
func (s *Store) AdmitProcedureRun(
	...
	for position, target := range targets {
		...
		if workflow.ID == "" {
			workflow = protocol.ProcedureWorkflowSnapshot(snapshot.Name, snapshot.Prompt)
		}
		resolvedPrompt := resolveTaskWorkflowPrompt(snapshot.Prompt, workflow, snapshot.WorkflowID != "" && snapshot.WorkflowID != "procedure")
