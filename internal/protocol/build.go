...
If .factory/workflows/implement.md exists, read and follow that repository implementation workflow before acting.

For GitHub issue work, re-read the live issue before making changes. Continue only while it still has the needs-agent label and its GitHub Project status is Ready.

Create or update one pull request, handle its CI and review findings, and push the final committed work to the immutable Factory publish branch supplied for this Work. When enabled, Factory runs the independent review gate after this executor stage.
`
...
type BuildRequest struct {
	...
	WorkflowSpecified   bool     `json:"workflow_specified"`
...
func NormalizeBuildRequest(input BuildRequest) (NormalizedBuildInput, error) {
	...
	value := NormalizedBuildInput{
		...
		WorkflowSpecified:   input.WorkflowSpecified,
	...
	if input.WorkflowSpecified {
		workflow, err := NormalizeRepositoryWorkflowID(input.Workflow)
		if err != nil {
			return NormalizedBuildInput{}, err
		}
		value.Workflow = workflow
	} else if strings.TrimSpace(input.Workflow) != "" {
		return NormalizedBuildInput{}, errors.New("workflow_specified must be true when workflow is provided")
	}
...
func BuildCallerFingerprint(input NormalizedBuildInput) ([]byte, error) {
	...
	encoded, err := json.Marshal(struct {
		...
		RuntimeSpecified: input.RuntimeSpecified, Runtime: input.Runtime,
		WorkflowSpecified: input.WorkflowSpecified, Workflow: input.Workflow, Rebuild: input.Rebuild,
	})
	if err != nil {
		return nil, err
	}
...
func CanonicalBuildRequest(input BuildRequest) (BuildRequest, NormalizedBuildInput, []byte, error) {
	...
	request := BuildRequest{
		...
		WorkflowSpecified:   normalized.WorkflowSpecified,
