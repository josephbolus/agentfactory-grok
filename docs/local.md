...
```sh
make run ~/.factory/claude-worker.toml
```
...
```sh
FACTORY_LISTEN=127.0.0.1:7444 make run

FACTORY_DATA_HOME=/srv/factory \
  make run /srv/factory/worker.toml
```
...
- confirm `make run` is still running;
