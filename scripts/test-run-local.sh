...
if ! printf '%s\n' "$output" |
  grep -q "Agent Factory worker did not register as healthy within 1 seconds"; then
  echo "launcher did not explain unhealthy readiness failure" >&2
  echo "$output" >&2
  exit 1
fi
if printf '%s\n' "$output" | grep -q "Agent Factory is ready"; then
  echo "launcher reported an unhealthy worker as ready" >&2
  echo "$output" >&2
  exit 1
fi
if [ -e "$temporary/retired/missing" ]; then
  echo "launcher created an Agent Factory data root below retired state before validation" >&2
  exit 1
fi
...
set -e
if [ "$status" -ne 1 ] ||
  ! printf '%s\n' "$output" | grep -q "Agent Factory server stopped during worker startup"; then
...
async function verifySignal(signal, suffix, listenVariable) {
  ...
  const capture = (chunk) => {
    output += chunk;
    if (!signalled && output.includes("Agent Factory is ready")) {
      signalled = true;
  ...
  if (!output.includes(`Agent Factory is ready at http://127.0.0.1:${port}/`)) {
...
echo "Agent Factory launcher honors bootstrap listen settings, rejects unhealthy workers, retired-state writes, server loss, and stalled readiness responses; signals stop cleanly."
