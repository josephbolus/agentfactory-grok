...
func TestPipelineAPIListsSummariesAndUpdatesDetails(t *testing.T) {
	...
	rejected := doJSON(http.MethodPost, "/api/v1/pipelines", protocol.SavePipelineRequest{
		Name: "Plan and build",
	...
	if rejected.Code != http.StatusBadRequest || !strings.Contains(rejected.Body.String(), "pipeline_sdlc_removed") {
		t.Fatalf("multi-stage create = %d %s", rejected.Code, rejected.Body.String())
	}
	...
	if len(summary.Stages) != 1 || summary.Stages[0].Prompt != "" {
	...
	if detail.Stages[0].Prompt != "{{ task.prompt }}" {
