...
func TestServerBootstrapConfigIsOptionalAndResolvesRelativeDatabase(t *testing.T) {
	...
	if config.Listen != "127.0.0.1:7447" || config.Database != filepath.Join(dataRoot, "state", "factory.sqlite3") || config.DefaultBuildRuntime != "pi" || config.GitHubIssuePollInterval != "45s" {
