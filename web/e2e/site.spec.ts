...
test("internal site explains Agent Factory", async ({ page }) => {
  const failures = capturePageFailures(page);
  await page.goto(siteURL, { waitUntil: "networkidle" });

  await expect(page).toHaveTitle("Agent Factory | A control plane for coding agents");
  await expect(page.getByRole("heading", { level: 1, name: "Software work, under control." })).toBeVisible();
  await expect(page.getByText("Agent Factory is a control plane for coding agents.")).toBeVisible();
  await expect(page.getByText("Approved internal source control").first()).toBeVisible();
...
test("internal site text meets WCAG AA contrast", async ({ page }) => {
  await page.goto(siteURL);
  expect(await findContrastFailures(page)).toEqual([]);
});

test("internal site is keyboard accessible", async ({ page }) => {
  await page.goto(siteURL);
...
test("internal site preserves the product window at intermediate widths", async ({ page }) => {
...
test("internal site remains usable at 390 pixels", async ({ page }) => {
  const failures = capturePageFailures(page);
