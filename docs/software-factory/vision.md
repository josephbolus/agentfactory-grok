...
Agent Factory is the control plane for software work performed by coding agents. A
developer gives Agent Factory one work item or a repository fleet. Agent Factory queues the
...
Agent Factory does not try to become a better coding agent. Pi, Codex, Claude Code,
and future runtimes already reason about repositories, use tools, create
subagents, test changes, and open pull requests. Agent Factory makes those agents
...
its outcome through a small Agent Factory capability instead of leaving Agent Factory to
interpret arbitrary text.

## Core experience

Agent Factory has five operator concepts:
...
repository, use tools or subagents, and satisfy that stage's prompt. Agent Factory
...
Agent Factory injects a short stage wrapper and an Attempt-scoped update tool:
...
The agent supplies semantic judgment. Agent Factory supplies durable state,
...
## Where Agent Factory is better

An interactive coding-agent terminal remains better for exploration, difficult
product choices, and one high-touch task. Agent Factory is better when a developer
...
Agent Factory succeeds when it reduces developer attention per ready pull request.
It fails when a developer still has to open, prompt, and monitor every agent
session individually.

## Product principles

1. Agent Factory owns coordination; coding agents own engineering judgment.
...
10. Agent Factory adds a durable outer loop and explicit agent boundaries, not a new
...
graphs, and a new Agent Factory coding agent are not required to test the product
thesis.

## Measures of progress

Agent Factory is moving toward this vision when a developer can:
...
- identify the exact Pipeline, Agent Factory-supplied context, source reference,
